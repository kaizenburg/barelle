					...
					<div id="aside" class="col-md-3">
						...
						<div class="aside">
							...
							<div class="checkbox-filter">
								...
								<div v-for="(category,index) in categories" :key="index" class="input-checkbox">
									<input type="checkbox"   @change="loadProducts" :id="'category_' + index" :value="category.id" v-model="selectedCategories" >
						...
						<div class="aside">
							...
							<div class="checkbox-filter">
								<div v-for="(subcategory,index) in subcategories" :key="index" class="input-checkbox">
									<input  type="checkbox" checked @change="loadProducts" :value="subcategory.id" v-model="selectedSubcategories" :id="'subcategory_' + index">
								...
								<div class="product">
									...
									<div class="product-body">
										...
										<div class="product-btns">
											...
											<button class="quick-view"> <router-link :to="{ name: 'product', params: {slug: product.slug} }"><i class="fa fa-eye"></i></router-link><span class="tooltipp"> view</span></button>
										</div>
									</div>
	...
	methods: {
      ...
      decrement() {
	   ...
	   selectAttribute(index) {
		...
		loadSubcategories() {
			axios.get('/api/browse/subcategories').then(res => {
				this.subcategories = res.data.subcategories;
			}).catch(err => console.log(err))
		},
		loadProducts(page = 1) {
			
			axios.get('/api/browse/?selectedCategories='
			 +  this.selectedCategories.toString() 
			+ '&selectedSubcategories=' + this.selectedSubcategories.toString()
			+ '&page=' + page
			+ '&sortby=' + this.sortby
			).then(res => {
